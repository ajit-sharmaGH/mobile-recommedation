...
function App() {
  ...
  const mobileCompanyArray = Object.keys(mobileItems);
  function companyNameClickHandler(mobileCompany) {
    setSelectedMobile(mobileCompany);
  }
  return (
    <div>
      <h2>Best Mobiles</h2>
      <h3>You can see best phones of the following company</h3>
      <div className="container">
        {mobileCompanyArray.map((mobileCompany) => {
          return (
            <button onClick={() => companyNameClickHandler(mobileCompany)}>
              {mobileCompany}
            </button>
          );
        })}
